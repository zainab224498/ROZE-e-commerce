...
import AccessoriesFilter from '../../components/filter/AccessoriesFilter'

function Accessories() {
    const context = useContext(myContext)
    const { mode, accessory, searchkey, setSearchkey, filterType, setFilterType,
    ...
    return (
        <Layout>
            <div>
                <h1 class="flex justify-center items-center text-3xl font-medium title-font my-10 text-gray-900" style={{ color: mode === 'dark' ? 'white' : '' }}>Accessories</h1>
                <AccessoriesFilter />
                                    ...
                                    return (
                                        <div key={index} className="p-4 lg:w-1/4 md:w-1/2 drop-shadow-lg " >
                                            <div className="h-full border-2 hover:shadow-gray-100 hover:shadow-2xl transition-shadow duration-300 ease-in-out    border-gray-200 border-opacity-60 rounded-2xl overflow-hidden" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
                                                <div onClick={() => window.location.href = `/accessoryinfo/${id}`} className="flex justify-center cursor-pointer" >
                                                ...
                                                <div className="p-5 border-t-2">
                                                    ...
                                                    <h1 onClick={() => window.location.href = `/accessoryinfo/${id}`} className="title-font text-lg font-medium text-gray-900 mb-3 hover:cursor-pointer" style={{ color: mode === 'dark' ? 'white' : '', }}>{title}</h1>
